...
To run the ravend tests manually, launch `src/test/test_th3`. To recompile
...
To run the raven-qt tests manually, launch `src/qt/test/test_th3-qt`
...
By default the log messages from the TH3 Core application are not echoed 
when running the unit tests.  If it is desired to print this log data change 
the following from 'false' to 'true' in the `test_th3.cpp` file and uncomment
...
Run `test_th3 --list_content` to get a full list of available unit tests.

To run just the 'getarg_tests' (verbosely):

    test_th3 --run_test=getarg_tests

... or to run just the doubledash test:

    test_th3 --run_test=getarg_tests/doubledash_test
...
The build system is setup to compile an executable called `test_th3`
that runs all of the unit tests.  The main source file is called
test_th3.cpp. To add a new unit test file to our test suite you need 
